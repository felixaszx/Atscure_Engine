...
namespace as
{
    Image::Image(const VkImageCreateInfo& image_info, const VmaAllocationCreateInfo& alloc_info)
    {
    ...
    void Image::create_image_view(const VkImageViewCreateInfo& view_info)
    {
                                                 ...
                                                 const std::vector<VkImageAspectFlags>& aspects)
    {
        ...
        for (int i = 0; i < attachments.size(); i++)
        {
            ...
            VmaAllocationInfo finish_info{};
            attachments[i] = std::addressof(device.link(new Image(create_info, alloc_info)));
            ...
            attachments[i]->create_image_view(view_info);
        }
